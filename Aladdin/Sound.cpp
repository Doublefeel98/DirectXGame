...
void Sound::playOnce(const char* fileName, std::string name) {
	loadSound(fileName, name);
	play(name, false, 1);
}
void Sound::playRep(const char* fileName, std::string name) {
	loadSound(fileName, name);
	play(name, true, 1);
}
void Sound::Update() {
	//int aladdinState = Aladdin::GetInstance()->GetState;
