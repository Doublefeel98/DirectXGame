...
int Game::InitWindow(int nCmdShow)
{
	...
	int windowStyle;
	if (G_IsFullScreen) {
		windowStyle = WS_EX_TOPMOST | WS_VISIBLE | WS_POPUP; //d?ng fullscreen
	}
	else {
		windowStyle = WS_OVERLAPPEDWINDOW; //d?ng c?a s?
	}
...
void Game::ProcessKeyBoard()
{
	...
	// Scan through allata, check if the key is pressed or released
...
void Game::InitGame()
{
	...
	InitDirectX();
	OutputDebugString("init direct X");
	InitKeyboard();
	OutputDebugString("Keyboard");
	LoadResources(G_Device);
	OutputDebugString("Load Resource");
}
...
void Game::GameRun()
{
	...
	while (game_is_runing)
	{
		...
		{
			frame_start = now;
			OutputDebugString("REnderFrame \n");
			RenderFrame();
			OutputDebugString("ProcessKeyBoard \n");
			ProcessKeyBoard();
